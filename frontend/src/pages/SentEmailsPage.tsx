...
function getStatusClasses(status: SentEmail['status']) {
  switch (status) {
    case 'SENT':
    ...
    try {
      ...
      setError(
        error instanceof Error
          ? error.message
          : 'Unable to load sent emails right now.',
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Emails that have already been processed by the scheduler.
          </p>
        </div>
      ...
      {!isLoading && !error && emails.length === 0 ? (
        <section className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-slate-500">
            Successfully sent or failed emails will appear here.
          </p>
        </section>
      ) : null}
                  ...
                  >
                    ...
                    <td className="px-4 py-4 sm:px-6">
                      {email.status === 'FAILED' && email.error ? (
                        <p className="max-w-xs text-sm text-rose-600">
                          {email.error}
                        </p>
                      ) : (
